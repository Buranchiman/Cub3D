...
int	monster_count(t_data *data)
{
	...
	if (monsters_count > 0)
	{
		...
		data->tab_monsters[0].count = monsters_count;
	}
	else
...
void	valid_moves(t_data *data, t_point tab_monster_tmp, int i)
{
	...
	if (data->map[ty][tx] == '1'
		...
		return ;
	j = 0;
	while (data->tab_monsters && j < data->tab_monsters[0].count)
	{
...
void	monsters_move(t_data *data)
{
	...
	i = 0;
	if (!data->tab_monsters || data->tab_monsters[0].count == 0)
		return ;
	while (i < data->tab_monsters[0].count)
	{
		...
		if ((data->player_pos.x - data->tab_monsters[i].pos.x) < 0)
			tab_monster_tmp.x = data->tab_monsters[i].pos.x - 0.3;
